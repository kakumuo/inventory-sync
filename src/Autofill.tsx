import { DepopModelResponseFields, DepopModelResponseObject } from "./generators/DepopGenerator"
...
export class DepopAutofillContext implements AutofillContext<DepopModelResponseObject>{
    ...
    constructor(targetData:DepopModelResponseObject) {
        this.targetData = targetData
        // this.fillOrder = ['[id=description]', '[id=listingCategories__category__select]', '[id=listingCategories__subcategory__select]', '[id$=-type-attribute__select]', '[id=dress-length-attribute__select]', '[id$=-style-attribute__select]', '[id=brand-attribute__select]', '[id=occasion-attribute__select]', '[id=material-attribute__select]', '[id=bodyfit-attribute__select]', '[id=condition-attribute__select]', '[id=createProductSizes__sizeRow0__size__select]', '[id=listingSelect__listing__colour__select]', '[id=listingSelect__source__select]', '[id=listingSelect__age__select]', '[id=listingSelect__style__select]']
        this.mapping = { 
            ...
            '[id=brand-attribute__select]': () => this.dropdownSelect('[id=brand-attribute__select]', this.targetData.brand),
            '[id=condition-attribute__select]': () => this.dropdownSelect('[id=condition-attribute__select]', this.targetData.condition),
    ...
    private async dropdownSelect(selector:string, data:string|undefined) {
        ...
        const options:NodeListOf<HTMLInputElement> = document.querySelectorAll('[class*="option"]')
        options.forEach(option => option.innerText.toLowerCase() == data.toLowerCase() ? option.click() : null)
    ...
    private async dropdownMultiSelect(selector:string, data:string[]|undefined) {
        ...
        for(const targetData of data){
            ...
            const options:NodeListOf<HTMLInputElement> = document.querySelectorAll('[class*="option"]')
            options.forEach(option => option.innerText.toLowerCase() == targetData.toLowerCase() ? option.click() : null)
...
export class ListingAutofiller {
    ...
    async autofill(){
        ...
        //FIXME: brand and condition not filling
